...
A: 刺激的数字峰值限幅为 -10 dBFS，响度按唤醒度映射到 [-30, -10] dBFS RMS（高唤醒→更响）。实际听到多响取决于播放设备与系统音量，请用界面底部音量滑块从低往高调；软件无法保证具体声压级。
...
A: 可用。点击左侧「开始录音」即开始采集（界面实时显示已录制秒数，上限 60 秒），再次点击或点「停止/重置」结束，录制的音频会自动落盘并触发完整分析。从下拉框可选择麦克风设备。
