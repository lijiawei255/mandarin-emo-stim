...
**v0.1.0 结果摘要**（详见 evaluation.md）：7 类表演型情绪的 V-A 相对排序 8/8 成对检验通过（效价 Spearman ρ = 0.69，唤醒 0.32）；
象限准确率 0.52（随机 0.25，多数类 0.41）；**存在系统性负面偏置**（中性朗读语音 62% 被判入 Q3）；
**多模态融合在该语料上未优于 emotion2vec 单模态**（0.52 vs 0.55），去掉文本语义支路反而升到 0.62；动态权重在录音棚条件下从未触发。
