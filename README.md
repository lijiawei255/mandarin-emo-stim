...
Mandarin-EmoStim 实现了一条本地的**测量闭环**：**说话 → 情绪量化 → 差异化声刺激 → 再说话（后测）**。
v0.5 的会话模式把受试者的多段录音合成带惯性的状态估计，并把前测、刺激、后测串成试次导出，
用于观察刺激前后的变化；跨试次如何调整刺激由实验者决定，没有自动策略。
...
**会话模式**（v0.5）把多段录音合成带惯性的状态估计：序列模拟中象限翻转率从 55% 降到 0，末段准确率略升；前测、刺激、后测串成试次导出，用于观察刺激前后的变化。
