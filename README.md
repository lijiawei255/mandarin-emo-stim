...
**v0.2.0 结果摘要**（详见 evaluation.md，含 v0.1 对照）：7 类表演型情绪的 V-A 相对排序 8/8 成对检验通过（效价 Spearman ρ = 0.79，唤醒 0.31）；
象限准确率全体 0.68、按说话人划分的 test 集 0.63（随机 0.25，多数类 0.41；v0.1 为 0.52）；中性校准后中性语音的负面偏置基本消除（negative μ 0.51，v0.1 为 0.55）；
**多模态融合在 v0.2 优于 emotion2vec 单模态**（0.68 vs 0.61）；**唤醒度判别仍弱**（sad 多数被判入 Q2）；动态权重在录音棚条件下从未触发。
