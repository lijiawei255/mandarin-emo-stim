...
合成流程的每一步都有心理声学依据：
  ...
  1. 基础谐和音：叠加正弦分量（按 harmony 定义）。协和音程（如大三和弦 1:1.25:1.5）
     听感稳定明亮，不协和（如密集整数泛音）制造紧张感——对应不同情绪象限。
     振幅按 1/n 衰减模拟自然泛音衰减。
  2. 频谱塑形（带通滤波）：把频谱重心调整到目标 sc，控制音色明暗（高频亮、低频暗）。
  3. 振幅调制（脉冲率 pr）：用正弦调制器对载波做 AM，pr 即调制频率。
     慢 pr（~0.3Hz）接近呼吸节律，有平复引导作用；快 pr（~4Hz）有激活/振奋感。
  4. ADSR 包络：起音时间控制「冲击感」vs「柔和感」（陡起音=冲击，缓起音=柔和）。
  5. 粉噪混合：粉噪（1/f 频谱）比白噪更柔和，用于能量遮蔽与频谱填充；其
     「放松」效应缺乏直接证据，本项目不作此主张（见 research_notes §4.4）。
